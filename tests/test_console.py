...
class TestCuisineCommand(unittest.TestCase):
    ...
    """
    @unittest.skipIf(
        os.getenv('CUISINE_TYPE_STORAGE') == 'db', 'FileStorage test')
    def test_fs_create(self):
        """Tests the create command with the file storage.
        """
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = CuisineConsole()
            cons.onecmd('create Recipe title="Chicken_masala"')
            mdl_id = cout.getvalue().strip()
            clear_stream(cout)
            self.assertIn('Recipe.{}'.format(mdl_id), storage.all().keys())
            cons.onecmd('show Recipe {}'.format(mdl_id))
            self.assertIn("'title': 'Chicken masala'", cout.getvalue().strip())
            clear_stream(cout)
